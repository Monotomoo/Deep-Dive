# Deep Dive · V1.1 — session checkpoint (v0.23 · the truth batch)

## Current state (2026-08-20)
**LIVE + CLOUD-SYNCED.** `deep-dive-amber.vercel.app`, git-linked auto-deploy from `main`. Supabase shared-crew project (invite-only magic-link, one shared doc, verified writes, dirty-flag refresh protection, DB guard trigger against stale-build tabs — SQL in `SUPABASE.md`). **Storage key `deep-dive-dashboard-v17`; content generation `scenarioSeedVersion = 5`** (gates a wholesale refresh of scenario/arcs/shoots/interviews/records/events/topics/usaTrip/calendar/scenarios/shootDays/milestones/schedulePhases on any older doc, incl. the cloud one). Sidebar shows a build stamp (`__BUILD_TS__`). Sign-out is device-local. Simple/Full sidebar toggle (per-device). **Open items:** repo still public with private docs in history at `1078582` (make private!); crew not yet invited; custom SMTP pending (built-in sender ≈2 emails/hour); Zsófia's Lastovo WR depth TBC; people roles (Andre/Toma/Denis/Mikey/Michael/Mariano) TBC; Day-3 build = Scenario PDF export.

### v0.23 · the truth batch (2026-08-20)
Post-audit fix-everything pass (6-agent audit, 52 findings). **Story:** Part 10 (the coda — Story 2's ending), Vito's-dive arc = mentioned at Lastovo, reveal in USA/Cyprus **to be decided with Vito** (gen 6), two-beat 2023 resolution (HoF = public, Rijeka studio = personal; dedicated `shoot-2023-studio`), Cyprus rebuilt as a climax (27.9–7.10, story beats, planned duo interview), Krk quotes surfaced, episode chips (`episodeHint`) on all parts, planned `int-usa-vito` + `int-cyp-pero-zso`, Molchanov window 15–22.9 (+ calendar event). **Dates:** HoF = **26.9.2026** (event/timeline/day-26 fixed), training camp 15–26.9, sicily-records from 9.10, Philippines 15.3–30.4.2027, post-production re-dated after Philippines (lock 31.8.27, pre-Sundance), milestones/phases rebuilt for all 10 shoots, USA stop-nights fixed (19→26). **Money:** costs gained Contingency 50 / Finishing+delivery 40 / Reserve 40 → realistic spend = raise = €290k (lean 190/190, ambitious 430/430); pitch budget card pinned to Realistic with honest labels ("budget"/"financing plan"); "two of seven shoots" → "three of ten" everywhere. **Hygiene:** sync-check button, `__ddDebug`, `probeWrite`/`authInfo` removed; stale subtitles fixed; Krk/Lastovo seed shoot-days added (Gap Radar contradiction noise cleared); map Sicily coords de-stacked.

### v0.18–v0.22 (2026-07-15 → 2026-08-20, summarized)
Shared-crew cloud (one doc, invite-only, live-refresh) → Gap Radar + Overview map + backup/export/snapshots + crew comments → Simple/Full mode + The Money board (editable, Tomo's real numbers) → The Scenario (4 connected stories × 9 parts, fully editable incl. link pickers) → sync saga fixes (dead pushes, fast-refresh dirty flag, verified writes, hydrate-echo guard, device-local sign-out, DB guard trigger).
